...
export const getVideoDetails = createAsyncThunk(
  ...
  async (id: string) => {
    const {
      ...
      `${YOUTUBE_API_URL}/videos?key=${_APIkey}&part=snippet,statistics&type=video&id=${id}`
    );
...
const parseData = async (item: {
  ...
  const {
    ...
    `${YOUTUBE_API_URL}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${_APIkey}`
  );
